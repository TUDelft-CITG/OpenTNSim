...
class IsLockLineUpArea(HasResource, Identifiable, Log):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Lay-Out
        self.line_up_area = {
            node: simpy.Resource(self.env, capacity=2),
        }
...
class IsLock(HasResource, HasContainer, Identifiable, Log):
    """Mixin class: Something has lock object properties
    properties in meters
    operation in seconds
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(capacity = lock_length, level = lock_length,*args, **kwargs)
        """Initialization"""
    ...
    def move(self):
        ...
        for node in enumerate(self.route):
                                ...
                                if 'Lock' in self.env.FG.nodes[r].keys():
                                    ...
                                    for lock2 in locks:
                                        if lock2.name == self.lock_name:
                                            ...
                                            for r5 in self.route[loc:]:
                                                if r5 == lock2.node_1:
                                                    self.dir = 1.0
                                                    break
                                                elif r5 == lock2.node_3:
                                                    self.dir = 2.0
                                                    break

                                            access_lock_length = lock2.container.get(self.length)
                                            self.lock_dist = lock2.container.level + 0.5*self.length
                                            yield access_lock_length

                                            departure_lock_length = lock2.container.put(self.length)
            ...
            if "Lock" in self.env.FG.nodes[origin].keys():
                ...
                for lock in locks:
                    if lock.name == self.lock_name:
                        self.log_entry("Passing lock start", self.env.now, 0, shapely.geometry.Point(self.lock_pos_lat,self.lock_pos_lon))
