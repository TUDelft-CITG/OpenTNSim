...
class VesselTrafficService:
    ...
    def __init__(self,hydrodynamic_data,vessel_speed_data,*args,**kwargs):
    ...
    def provide_speed(self,vessel,edge):
        if edge in list(self.vessel_speeds.edge.values):
    ...
    def provide_tidal_window_restriction(self, vessel, route, specifications, node):
        ...
        for restriction_class in enumerate(specifications[0]):
            ...
            for restriction_type in enumerate(restriction_class[1]):
                ...
                if restriction_type[1].find('Draught') != -1: value = getattr(vessel, 'T')
                if restriction_type[1].find('Beam') != -1: value = getattr(vessel, 'B')
                if restriction_type[1].find('UKC') != -1: value,_,_,_,_,_ = self.provide_ukc_clearance(vessel,node,delay)
    ...
    def provide_horizontal_tidal_windows(self, vessel, route, time_start, time_end, delay=0, plot=False):
        ...
        def calculate_horizontal_tidal_window(vessel, time_start_index, time_end_index, hydrodynamic_data, critical_limits=[],cross_current_limit_dataframe=pd.DataFrame(), flood=True, ebb=True,decreasing=False):
            ...
            roots_cv = [root for root in roots if root >= currents_time[0].astype(float) and root <= currents_time[-1].astype(float)]
            ...
            if isinstance(cross_current_limit_dataframe,pd.DataFrame) and not cross_current_limit_dataframe.empty:
                critical_limit = np.interp(currents_time.astype('float'), cross_current_limit_dataframe.index.to_numpy().astype('float'), cross_current_limit_dataframe.Limit.to_numpy())
                ...
                critical_current_velocity = np.interp(np.array(current_intersections).astype('float'), currents_time.astype('float'),critical_limit)
                horizontal_tidal_accessibility = pd.DataFrame(data=critical_current_velocity, columns=['Limit'], index=current_intersections)
            elif isinstance(critical_limits,list):
                critical_current_velocity = []
                for critical_limit in critical_limits:
                    idx = np.argwhere(np.diff(np.sign([current_velocity - critical_limit for current_velocity in currents_data]))).flatten()
                    roots = currents_time[idx]
                    current_intersections.extend(roots.astype(dtype='datetime64[ns]'))
            ...
            for period_nr, (tidal_period_start, tidal_period_end) in enumerate(zip(tidal_periods[:-1], tidal_periods[1:])):
                ...
                if tidal_period == 'Rising': tidal_period = 'Flood'
                if tidal_period == 'Falling': tidal_period = 'Ebb'
                tidal_period_start = tidal_period_start[0]
                tidal_period_end = tidal_period_end[0]
                horizontal_tidal_accessibility.loc[horizontal_tidal_accessibility[(horizontal_tidal_accessibility.index >= tidal_period_start) & (horizontal_tidal_accessibility.index <= tidal_period_end)].index, 'Period'] = tidal_period
                horizontal_tidal_accessibility.loc[horizontal_tidal_accessibility[(horizontal_tidal_accessibility.index >= tidal_period_start) & (horizontal_tidal_accessibility.index <= tidal_period_end)].index, 'Period_nr'] = period_nr
            ...
            if decreasing:
                ...
                if flood:
                    selected_horizontal_tidal_accessibility = pd.concat([selected_horizontal_tidal_accessibility,horizontal_tidal_accessibility[horizontal_tidal_accessibility.Period == 'Flood']])
                if ebb:
                    selected_horizontal_tidal_accessibility = pd.concat([selected_horizontal_tidal_accessibility,horizontal_tidal_accessibility[horizontal_tidal_accessibility.Period == 'Ebb']])
                horizontal_tidal_accessibility = selected_horizontal_tidal_accessibility.sort_index()
                horizontal_tidal_accessibility = horizontal_tidal_accessibility.loc[horizontal_tidal_accessibility.index.drop_duplicates(keep=False)]
            ...
            if decreasing:
                ...
                end_time_windows = []
                for period_nr in [idx for idx, count in horizontal_tidal_accessibility.value_counts('Period_nr').items() if count == 2]:
                ...
                missing_tides = set(list(np.arange(tide_number, number_of_tidal_periods, 2))) - set(list(dict.fromkeys(horizontal_tidal_accessibility[horizontal_tidal_accessibility.Period == 'Flood'].Period_nr)))
                for tide_index in missing_tides:
                    ...
                    current_velocity = np.interp(closing_time, [currents_time[previous_index], currents_time[next_index]], [currents_data[previous_index], currents_data[next_index]])
                ...
                for period_nr, count in [(idx, count) for idx, count in horizontal_tidal_accessibility.value_counts('Period_nr').items() if count != 2]:
                    if count == 1:
                        for iloc, (loc, info) in enumerate(horizontal_tidal_accessibility[horizontal_tidal_accessibility.Period_nr == period_nr].iterrows()):
                        ...
                        horizontal_tidal_accessibility = horizontal_tidal_accessibility.drop(horizontal_tidal_accessibility[(horizontal_tidal_accessibility.Period_nr == period_nr) & (horizontal_tidal_accessibility.Limit == np.max(critical_limits))].index[:-1])
                        horizontal_tidal_accessibility = horizontal_tidal_accessibility.drop(horizontal_tidal_accessibility[(horizontal_tidal_accessibility.Period_nr == period_nr) & (horizontal_tidal_accessibility.Limit == np.min(critical_limits))].index[:-1])
                ...
                for period_nr in [idx for idx, count in horizontal_tidal_accessibility.value_counts('Period_nr').items() if count % 2]:
                    for loc, info in horizontal_tidal_accessibility[horizontal_tidal_accessibility.Period_nr == period_nr].iterrows():
                        if currents_data[list(currents_time).index(loc) - 1] < info.Limit and currents_data[list(currents_time).index(loc) + 1] < info.Limit:
            ...
            if not horizontal_tidal_accessibility.empty:
                if not decreasing:
                    horizontal_tidal_accessibility.loc[list(horizontal_tidal_accessibility.index)[1::2], 'Accessibility'] = 'Accessible'
                    horizontal_tidal_accessibility.loc[list(horizontal_tidal_accessibility.index)[::2], 'Accessibility'] = 'Inaccessible'
                    limiting_currents = np.interp(horizontal_tidal_accessibility.index.to_numpy().astype(float),cross_current_limit_dataframe.index.to_numpy().astype(float),cross_current_limit_dataframe.Limit.to_numpy())
                    horizontal_tidal_accessibility['Accessibility'] = ['Accessible' if interpcur < limit else accessibility for accessibility, interpcur, limit in zip(horizontal_tidal_accessibility.Accessibility.to_numpy(),horizontal_tidal_accessibility.Limit.to_numpy(), limiting_currents)]
                    horizontal_tidal_accessibility['Limit'] = [0 if interpcur < limit else limit for interpcur, limit in zip(horizontal_tidal_accessibility.Limit.to_numpy(),limiting_currents)]
                    horizontal_tidal_accessibility = horizontal_tidal_accessibility[horizontal_tidal_accessibility.Limit != -1.0]
                else:
                    horizontal_tidal_accessibility.loc[list(horizontal_tidal_accessibility.index)[1::2], 'Accessibility'] = 'Inaccessible'
                    horizontal_tidal_accessibility.loc[list(horizontal_tidal_accessibility.index)[::2], 'Accessibility'] = 'Accessible'
        ...
        for route_index, node_name in enumerate(route):
            if 'Horizontal tidal restriction' in vessel.env.FG.nodes[node_name]['Info'].keys():
                ...
                if window_specifications.window_method == 'Maximum':
                    next_horizontal_tidal_accessibility,station = calculate_horizontal_tidal_window(vessel,time_start_index,time_end_index,hydrodynamic_data,cross_current_limit_dataframe=cross_current_limit)
                if window_specifications.window_method == 'Point-based':
                    if isinstance(window_specifications.current_velocity_values['Flood'], list) and not isinstance(window_specifications.current_velocity_values['Ebb'], list):
                        next_horizontal_tidal_accessibility,station = calculate_horizontal_tidal_window(vessel,time_start_index,time_end_index,hydrodynamic_data,critical_limits=cross_current_limit,ebb=False,decreasing=True)
                    elif isinstance(window_specifications.current_velocity_values['Ebb'], list) and not isinstance(window_specifications.current_velocity_values['Flood'], list):
                        next_horizontal_tidal_accessibility,station = calculate_horizontal_tidal_window(vessel,time_start_index,time_end_index,hydrodynamic_data,critical_limits=cross_current_limit,flood=False,decreasing=True)
                    else:
                        next_horizontal_tidal_accessibility,station = calculate_horizontal_tidal_window(vessel,time_start_index,time_end_index,hydrodynamic_data,critical_limits=cross_current_limit,decreasing=True)
                ...
                if horizontal_tidal_accessibility.empty:
                    ...
                    horizontal_tidal_accessibility = self.combine_tidal_windows(horizontal_tidal_accessibility, next_horizontal_tidal_accessibility)

        if horizontal_tidal_accessibility.empty or not horizontal_tidal_window:
            horizontal_tidal_accessibility = pd.DataFrame(columns=['Limit', 'Condition', 'Accessibility'])
            horizontal_tidal_accessibility.loc[np.datetime64(vessel.env.simulation_start.replace(tzinfo=None)), :] = [0,'Current velocity','Accessible']
            horizontal_tidal_accessibility.loc[np.datetime64(vessel.env.simulation_stop.replace(tzinfo=None)), :] = [0,'Current velocity','Inaccessible']
        else:
            if horizontal_tidal_accessibility.iloc[0].Accessibility == 'Inaccessible':
                horizontal_tidal_accessibility.loc[np.datetime64(vessel.env.simulation_start.replace(tzinfo=None)), :] = [0,'Current velocity','Accessible']
            else:
                horizontal_tidal_accessibility.loc[np.datetime64(vessel.env.simulation_start.replace(tzinfo=None)), :] = [0,'Current velocity','Inaccessible']
            if horizontal_tidal_accessibility.iloc[-1].Accessibility == 'Inaccessible':
                horizontal_tidal_accessibility.loc[np.datetime64(vessel.env.simulation_stop.replace(tzinfo=None)), :] = [0,'Current velocity','Accessible']
            else:
                horizontal_tidal_accessibility.loc[np.datetime64(vessel.env.simulation_stop.replace(tzinfo=None)), :] = [0,'Current velocity','Inaccessible']
    ...
    def combine_tidal_windows(self,tidal_window_1, tidal_window_2):
        accessibility_1 = tidal_window_1.iloc[[bisect.bisect_right(tidal_window_1.index, index) - 1 for index in tidal_window_2.index]].Accessibility.to_numpy()
        accessibility_2 = tidal_window_2.iloc[[bisect.bisect_right(tidal_window_2.index, index) - 1 for index in tidal_window_1.index]].Accessibility.to_numpy()
    ...
    def provide_tidal_windows(self,vessel,route,time_start,time_end,delay=0,plot=False):
        time_start_index = np.max([0,np.absolute(self.hydrodynamic_information.TIME.values - (time_start + np.timedelta64(int(delay), 's'))).argmin()-2])
        time_end_index = np.absolute(self.hydrodynamic_information.TIME.values - (time_end + np.timedelta64(int(delay),'s'))).argmin()
        ...
        if plot:
            ...
            for node, station in zip(horizontal_tidal_restriction_nodes, horizontal_tidal_restriction_stations):
                ...
                current_velocity, = ax_right.plot([time - horizontal_tidal_accessibility_time_correction for time in self.hydrodynamic_information.TIME.values[time_start_index:time_end_index]], governing_current_velocity,color='firebrick', linewidth=2)
                if window_specifications.window_method == 'Maximum':
            ...
            # Figure bounds
            ax.set_xlim(self.hydrodynamic_information.TIME.values[time_start_index], self.hydrodynamic_information.TIME.values[time_end_index-36])
            ax.set_ylim(np.min([np.floor(np.min(net_ukcs['min_net_ukc'].to_numpy())), -1.0]), np.max([np.ceil(np.max(net_ukcs['min_net_ukc'])),1.0]))

            # Calculate vertical and horizontal tidal windows
            vertical_tidal_window_polygons = []
            for window in vertical_tidal_windows:
                vertical_tidal_window_polygons.append(Polygon([Point((window[0] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[0]),
            ...
            for window in horizontal_tidal_windows:
                horizontal_tidal_window_polygons.append(Polygon([Point((window[0] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[0]),
                                                                 Point((window[0] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[1]),
                                                                 Point((window[1] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[1]),
                                                                 Point((window[1] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[0])]))
            tidal_window_polygons = []
            for window in tidal_windows:
                tidal_window_polygons.append(Polygon([Point((window[0] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[0]),
                                                      Point((window[0] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[1]),
                                                      Point((window[1] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[1]),
                                                      Point((window[1] - np.datetime64('1970-01-01')) / np.timedelta64(1, 's'), ax.get_ylim()[0])]))
            ...
            if not isinstance(vertical_tidal_window_polygons,Polygon):
                for polygon in vertical_tidal_window_polygons.geoms:
                    ...
                    vertical_tidal_window, = ax.fill(polygon_x, polygon_y, facecolor='C0', alpha=0.25, edgecolor='none')
            ...
            if not isinstance(horizontal_tidal_window_polygons, Polygon):
                for polygon in horizontal_tidal_window_polygons.geoms:
                    ...
                    for timestamp in polygon.exterior.xy[0]:
                        polygon_x.append(pd.Timestamp(datetime.datetime.fromtimestamp(timestamp, tz=pytz.utc)))
                    polygon_y = list(polygon.exterior.xy[1])
                    horizontal_tidal_window, = ax.fill(polygon_x, polygon_y, facecolor='firebrick', alpha=0.25,edgecolor='none')
            ...
            if window_specifications:
                if window_specifications.window_method == 'Maximum':
                    ax.legend([net_UKC, minimum_required_net_ukc, current_velocity, critical_current_velocity, vertical_tidal_window, horizontal_tidal_window, tidal_window],['Net UKC', 'Required net UKC', 'Current velocity', 'Vertical tidal windows', 'Horizontal tidal windows','Resulting tidal windows'],frameon=False, loc='upper left', bbox_to_anchor=(1.05, 1.0));
                else:
                    ax.legend([net_UKC, minimum_required_net_ukc, current_velocity, vertical_tidal_window, horizontal_tidal_window, tidal_window],['Net UKC', 'Required net UKC', 'Current velocity', 'Vertical tidal windows', 'Horizontal tidal windows','Resulting tidal windows'], frameon=False, loc='upper left', bbox_to_anchor=(1.05, 1.0));
            else:
