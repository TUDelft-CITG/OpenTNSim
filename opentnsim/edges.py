...
def pass_berth(env, vessel, edge, anchorage_info):
    ...
    assert 'unload_duration' in vessel.metadata, 'If you want to unload at a quay a ship should have an unload duration'
    berth = edge['Berth']

    # For the last edge:
    vessel.log_entry('Start unloading at Berth', env.now, {'vessel': vessel, 'berth': berth}, berth.geometry)
    # HasContainer -> Quay
    if isinstance(berth, opentnsim.core.HasContainer):
        berth.log_entry('Start unloading at Berth', env.now,  {'vessel': vessel, 'berth': berth, 'level': berth.container.level}, berth.geometry)
    # HasResource -> Jetty
    elif isinstance(berth, opentnsim.core.HasResource):
        berth.log_entry('Start unloading at Berth', env.now,  {'vessel': vessel, 'berth': berth, 'count': berth.resource.count}, berth.geometry)

    yield vessel.env.timeout(vessel.metadata['unload_duration'])
    vessel.log_entry('Stop unloading at Berth', env.now, {'vessel': vessel, 'berth': berth}, vessel.geometry)

    if isinstance(berth, opentnsim.core.HasContainer):
        berth.log_entry('Stop unloading at Berth', env.now, {'vessel': vessel, 'berth': berth, 'level': berth.container.level}, berth.geometry)
        # give back space on the Quay
        yield berth.container.put(vessel.length)
        berth.log_entry('Stop release reservation at Berth', env.now, {'vessel': vessel, 'berth': berth, 'level': berth.container.level}, berth.geometry)
    elif isinstance(berth, opentnsim.core.HasResource):
        berth.log_entry('Stop unloading at Berth', env.now, {'vessel': vessel, 'berth': berth, 'count': berth.resource.count}, berth.geometry)
        # release the Jetty
        # we should have requested this spot in the anchorage area, now we can release it.
        berth_req = anchorage_info.get('berth_req')
        assert berth_req is not None, f"Arrived at berth of type resource with berth request {berth_req}\n for vessel \n{vessel} at edge:\n{edge}, with info \n{anchorage_info}"
        yield berth.resource.release(berth_req)
        berth.log_entry('Stop release reservation at Berth', env.now, {'vessel': vessel, 'berth': berth, 'count': berth.resource.count}, berth.geometry)
...
def pass_anchorage(env, vessel, edge, route, graph, info):
    ...
    """
    assert len(route) > 2, 'If there is an anchorage in the route, the route should be at least 2 nodes long'
    ...
    anchorage.log_entry('Request spot at Anchorage', env.now, {"berth":  anchorage, "vessel": vessel, "count": anchorage.resource.count}, anchorage.geometry)
    ...
    with anchorage.resource.request() as anchor_req:
        ...
        assert isinstance(next_edge['Berth'], (opentnsim.core.HasContainer, opentnsim.core.HasResource))
        # Lookup the berth
        berth = next_edge['Berth']
        vessel.log_entry('Request spot at Berth', env.now, {"berth":  berth, "vessel": vessel, "length": vessel.length}, vessel.geometry)
        if isinstance(berth, opentnsim.core.HasContainer):
            # We need information from the ship to be able to moore on a quay
            assert isinstance(vessel, opentnsim.core.VesselProperties), "If you have quays (Berths with a container) in your network, all ships should have a length implemented with VesselPropertis"
            yield berth.container.get(vessel.length)
            berth.log_entry('Start assign reservation at Berth', env.now, {"berth":  berth, "vessel": vessel, "level": berth.container.level}, berth.geometry)
        elif isinstance(berth, opentnsim.core.HasResource):
            berth_req = berth.resource.request()
            # we need to release this request if we are done loading
            info['berth_req'] = berth_req
            yield berth_req
            berth.log_entry('Start assign reservation at Berth', env.now, {"berth":  berth, "vessel": vessel, "count": berth.resource.count}, berth.geometry)

    anchorage.log_entry('Release spot at Anchorage', env.now, {"berth":  anchorage, "vessel": vessel, "count": anchorage.resource.count}, anchorage.geometry)
