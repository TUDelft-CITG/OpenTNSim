...
def simulate(locations, vessels, graph, arrival_rate, n_ships, seed=42):
    ...
    locations, graph = opentnsim.fis.add_locations_and_graph_to_env(env, locations, graph)
    ...
    moved_out =sample.vessel.apply(
        lambda vessel: pd.DataFrame(vessel.log).iloc[-1].Message
    ).str.startswith('Moving out').sum()
