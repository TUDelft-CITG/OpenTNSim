...
def _get_vessels_from_planned_operation(lock_chamber, operation_index = None):
    ...
    selected_operation = operation_planning_lock[operation_planning_lock.operation_index == operation_index]
    if not selected_operation.empty:
        vessels = selected_operation.iloc[0].vessels.copy()
    return vessels
...
def _update_lock_vessel_planning(lock_complex, vessel_index, passage_information):
    ...
    for key, value in passage_information.items():
        ...
        if isinstance(value, pd.Timedelta):
            value = value.round('us') # round to us

        lock_complex.vessel_planning.at[int(vessel_index), key] = value
...
def _get_previous_assigned_vessel(lock_complex, lock_chamber, operation_index):
    ...
    operation_planning_lock = operation_planning[operation_planning.lock_chamber == lock_chamber.name]
    assigned_operation = operation_planning_lock[operation_planning_lock.operation_index == operation_index].iloc[-1]
...
def _update_operation_planning_for_delayed_arrival(lock_complex, vessel, operation_index, delay):
    ...
    delay = datetime.timedelta(seconds=delay).round("us")
    operation_planning = lock_complex.operation_planning
    operation_planning.loc[operation_index, 'time_lock_entry_start'] += delay
    operation_planning.loc[operation_index, 'time_potential_lock_gate_opening_stop'] += delay
...
def _update_future_lock_operations_by_lock_delay_previous_operation(lock_chamber, operation_index, lock_departure_information):
    ...
    operation_planning_lock = operation_planning[operation_planning.lock_chamber == lock_chamber.name]
    next_planned_operations = operation_planning_lock[operation_planning_lock.operation_index > operation_index]
...
def determine_if_gate_is_closed(lock_chamber, operation_index, direction, vessel = None, first_in_lock=False, between_arrivals=False):
    ...
    vessel_planning = lock_chamber.lock_complex.vessel_planning
    operation_planning_lock = operation_planning[operation_planning.lock_chamber == lock_chamber.name]
    operation_information = operation_planning_lock[operation_planning_lock.operation_index == operation_index].iloc[-1]
    vessels = operation_information.vessels
    vessel_index = 0
    ...
    if operation_index - 2 in operation_planning.index:
        previous_operation_information = operation_planning_lock[operation_planning_lock.operation_index == operation_index - 1].iloc[-1]
        previous_vessels = previous_operation_information.vessels
        last_lockage_was_empty = len(previous_vessels) == 0
    if last_lockage_was_empty:
        return False, None, None

    if not first_in_lock and vessel_index:
        previous_vessel_planning_index = (
            vessel_planning[vessel_planning.id == vessels[vessel_index - 1].id]
            .iloc[-1]
            .name
        )
    ...
    if first_in_lock:
        ...
        gate_required_to_be_open = operation_information.time_potential_lock_gate_opening_stop
    else:
