...
Anchorage = type('Anchorage', (opentnsim.core.Identifiable, opentnsim.core.Log, opentnsim.core.Locatable , opentnsim.core.HasResource, opentnsim.core.ExtraMetadata), {})
Quay = type('Quay', (opentnsim.core.Identifiable, opentnsim.core.Log, opentnsim.core.Locatable, opentnsim.core.HasContainer, opentnsim.core.ExtraMetadata), {})
Jetty = type('Jetty', (opentnsim.core.Identifiable, opentnsim.core.Log, opentnsim.core.Locatable, opentnsim.core.HasResource, opentnsim.core.ExtraMetadata), {})
Vessel = type('Vessel', (opentnsim.core.Identifiable, opentnsim.core.VesselProperties, opentnsim.core.Movable,  opentnsim.core.ExtraMetadata), {})
...
def load_example_locations(graph, anchorage_size=30):
    rows = [
        ...
        {"n": "8863919", "e": ("8864185", "8863919"), "berth_type": "Quay", "type": "Container", "name": "Container1", "channel": "Europa Haven", "terminal": "APM", "quay_length": 1600},
        {"n": "8865973", "e": ("8867633", "8865973"), "berth_type": "Quay", "type": "Container", "name": "Container2", "channel": "Amazonehaven", "terminal": "Hutchison Ports ECT Delta", "quay_length": 2000},

        # {"n": "8866119", "e": ("8865518", "8866119"), "berth_type": "Jetty", "type": "Bulk", "name": "Agri Bulk", "terminal": "EBS", "quay_length": 220, "capacity": 4},
        {"n": "8868049", "e": ("8861217", "8868049"), "berth_type": "Quay", "type": "Bulk", "name": "Bulk", "channel": "Mississippi Haven", "terminal": "EMO", "quay_length": 1350},

        # {"n": "8861397", "e": ("8860727", "8861397"), "berth_type": "Quay", "type": "Cargo", "name": "Cargo", "terminal": "Waalhaven Noordzijde", "quay_length": quay_length},
        {"n": "8868178", "e": ["8866999", "8868178"], "berth_type": "Jetty", "type": "Liquid", "name": "Liquid", "terminal": "Vopak Terminal Botlek", "channel": "3e Petroleumhaven", "capacity": 8},
        # Liquid bbotlek
        # anchorage has a capacity in number of spots
        {"n": "8867050", "e": ("8867050", "8863654"), "berth_type": "Anchorage", "type": "Anchorage", "name": "Anchorage", "capacity": anchorage_size},
        {"n": "8868117", "e": ("13174615", "8868117"), "berth_type": None, "type": "Origin", "name": "Origin"}
    ]
    geometry = [
        graph.edges[row['e']]['geometry']
        for row
        in rows
    ]
...
def plot_locations(graph, locations, center, zoom):
    ...
    columns = ["terminal", "type", "berth_type", "channel", "quay_length", "capacity", "geometry"]

    map = ipyleaflet.Map(basemap=ipyleaflet.basemaps.OpenStreetMap.BlackAndWhite, center=center, zoom=zoom)
    locations_data = ipyleaflet.GeoData(
        geo_dataframe=locations[columns],
        style={'color': '#ccf', 'opacity': 3, 'weight': 4, 'dashArray': '2', 'fillOpacity': 0.6},
    ...
    for terminal, color in zip(['Container1', 'Container2', 'Bulk', 'Liquid'], matplotlib.cm.Set1.colors):
        coords = []
    ...
    for i, row in locations.iterrows():
        marker = ipyleaflet.Marker(location=(row.geometry.centroid.y, row.geometry.centroid.x))
...
def add_locations_and_graph_to_env(env, locations, graph):
    ...
    anchorage = Anchorage(env=env, name='Anchorage', nr_resources=row['capacity'], geometry=row['geometry'])
    locations.at["Anchorage", "Berth"] = anchorage
    graph.edges[row['e']]['Anchorage'] = anchorage
    # add the quays
    for name, row in locations.loc[['Container1', 'Container2', 'Bulk', 'Liquid']].iterrows():
        #  Create a Quay with an initial available length of quay_length
        berth = None
        if row.berth_type == 'Quay':
            berth = Quay(env=env, name=name, capacity=row['quay_length'], level=row['quay_length'], geometry=row['geometry'])
        elif row.berth_type == 'Jetty':
            berth = Jetty(env=env, name=name, nr_resources=row['capacity'], geometry=row['geometry'])
        else:
...
def vessel_sample(vessels, locations, graph, n_ships, arrival_rate=1, seed=42, env=None):
    ...
    sample['IAT'] = M.rvs(n_ships)
    sample['AT'] = sample['IAT'].cumsum()

    # create vessel objects, needed for the simulation
    sample['vessel'] = sample.apply(row2vessel, axis=1, env=env, locations=locations, graph=graph)
...
def move(env, graph, vessel):
    ...
    assert current_location == start_location, f'Ship is at {current_location}, not at {start_location}'
    ...
    for i, e in enumerate(zip(route[:-1], route[1:])):
        ...
        if 'Anchorage' in edge:
            ...
            yield from opentnsim.edges.pass_anchorage(env, vessel, edge, graph=graph, info=anchorage_info, route=route[i:])
        ...
        # pass_berth
        if 'Berth' in edge:
            yield from opentnsim.edges.pass_berth(env, vessel, edge, anchorage_info=anchorage_info)
...
def locations_plot(locations):
    fig, axes = plt.subplots(nrows=5, figsize=(8, 10), sharex=True)

    log = pd.DataFrame(locations.loc['Anchorage'].Berth.log)
    log['count'] = log['Value'].apply(lambda x: x['count'])
    axes[0].plot(log['Timestamp'], log['count'])
    axes[0].set_title('Anchorage')
    axes[0].set_ylabel('Spots in use')


    for ax, (name, row) in zip(axes[1:], locations.loc[['Container1', 'Container2', 'Bulk']].iterrows()):
        berth = row['Berth']
    ...
    log = pd.DataFrame(locations.loc['Liquid'].Berth.log)
    log['count'] = log['Value'].apply(lambda x: x['count'])
    axes[4].plot(log['Timestamp'], log['count'])
    axes[4].set_title('Liquid')
    axes[4].set_ylabel('Spots in use');
    return fig, ax
