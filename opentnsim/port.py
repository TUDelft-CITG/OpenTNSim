    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.type == 'quay':
            ...
            capacity = len(jetty_locations)

            "Initialization"
            super().__init__(type=type,capacity=capacity,length=length, *args, **kwargs)
...
class PassTerminal:
    ...
    def waiting_time_for_tidal_window(vessel,route,delay=0,plot=False):
        ...
        accessible_windows, starting_node_tidal_window = vessel.env.vessel_traffic_service.provide_sail_in_times_tidal_window(vessel,route=route,delay=delay,plot=plot)
        ...
        return waiting_time, accessible_windows, starting_node_tidal_window
    ...
    def move_to_anchorage(vessel,node):
        ...
        node_anchorage = sorted_nodes_anchorages[np.argmin(sailing_times_to_anchorages)]
    ...
    def pass_anchorage(vessel, node):
        ...
        if type(vessel.waiting_time_in_anchorage) != np.float64:
            ...
            if vessel.env.now - arrival_time < vessel.metadata['max_waiting_time']:
                ...
                vessel.waiting_time_in_anchorage, vessel.sail_in_times, vessel.starting_node_tidal_window = PassTerminal.waiting_time_for_tidal_window(vessel, route=vessel.route_after_anchorage, delay=0, plot=True)
                vessel.etd = vessel.waiting_time_in_anchorage + vessel.env.now + vessel.sailing_time_to_terminal + vessel.metadata['t_l'] + 2 * vessel.metadata['t_b']
            ...
            yield vessel.env.timeout(vessel.waiting_time_in_anchorage) | vessel.env.timeout(vessel.metadata['max_waiting_time'])
            vessel.status = 'moving to terminal'

        if vessel.accessibility:
            vessel.bound = 'outbound'
            vessel.waiting_time_after_terminal, vessel.sail_out_times, vessel.starting_node_tidal_window = PassTerminal.waiting_time_for_tidal_window(vessel, route=vessel.route_after_terminal,delay=vessel.metadata['t_l'] + 2 * vessel.metadata['t_b'] + vessel.sailing_time_to_terminal, plot=True)
            vessel.bound = 'inbound'

            if vessel.waiting_time_after_terminal >= vessel.metadata['max_waiting_time']:
                vessel.accessibility = False
        ...
        if not vessel.accessibility:
            ...
            vessel.route.append(vessel.original_origin)
        else:
    ...
    def release_terminal_access(vessel,terminal,index,delay):
        ...
        # Else if the terminal is of type 'jetty': adjust number of vessels occupying a jetty, log the departure of this vessel and this number, and release the request of the vessel for the specific jetty
            ...
            if vessel.accessibility:
                terminal.log_entry("Departure of vessel", vessel.env.now+delay, len(terminal.resource.users)-1,nx.get_node_attributes(vessel.env.FG, "geometry")[index],)
            terminal.resource.release(vessel.access_terminal)
    ...
    def request_terminal_access(vessel, edge, node):
        ...
        ##TODO: add channel width to (FIS) graph
        # for edge in [[node1,node2] for node1,node2 in zip(vessel.route[:-1],vessel.route[1:])]
        #     if vessel.env.FG.edges[edge]['Info']['Width'] < vessel.B:
        #         vessel.accessibility = False
        ...
        if vessel.accessibility:
            ...
            if terminal.type == 'jetty':
                ## TODO: add jetty lengths and guaranteed depths and add a selection strategy of the jetty (parallel waiting, priority, etc.)
                ##TODO: include priority
                if len(terminal.resource.users) != terminal.resource.capacity-1 or (terminal.resource.queue == [] and vessel.env.now + vessel.sailing_time_to_terminal >= terminal.resource.users[0].obj.etd):
                    vessel.access_terminal = terminal.resource.request() #[vessel.index_jetty_position]
                    ...
                    vessel.waiting_time_in_anchorage = vessel.access_terminal = terminal.resource.request()
                    vessel.terminal_requested = True
            ...
            if not vessel.waiting_for_availability_terminal:
                vessel.bound = 'inbound'
                waiting_time, vessel.sail_in_times, vessel.starting_node_tidal_window = PassTerminal.waiting_time_for_tidal_window(vessel, route=vessel.route, delay=0, plot=True)
                if terminal.type == 'jetty':
                    vessel.etd += waiting_time
                elif terminal.type == 'quay':
                    vessel.etd += waiting_time
                ...
                if waiting_time and 'waiting_time_in_anchorage' not in dir(vessel):
                    vessel.waiting_time_in_anchorage = waiting_time
                    vessel.move_to_anchorage = True

            if vessel.move_to_anchorage:
                if vessel.terminal_request_accepted:
                    vessel.status = 'waiting in anchorage'
                yield from PassTerminal.move_to_anchorage(vessel, node)

        if not vessel.accessibility:
            ##TODO: move vessel offshore immediately, instead of going to the anchorage area
            yield from PassTerminal.move_to_anchorage(vessel, node)

        # If the vessel does not have to return to sea
        if vessel.accessibility:
            if vessel.move_to_anchorage:
                vessel.route_after_terminal = nx.dijkstra_path(vessel.env.FG, vessel.route_after_anchorage[-1],vessel.original_origin)
            else:
    ...
    def pass_terminal(vessel,edge):
        ...
        if terminal.type == 'quay':
            berth_position = vessel.quay_position

        elif terminal.type == 'jetty':
            berth_position = terminal.jetty_locations[[user.obj.id for user in terminal.resource.users].index(vessel.id)]

        [vessel.terminal_pos_lat, vessel.terminal_pos_lon, _] = vessel.wgs84.fwd(vessel.env.FG.nodes[edge[0]]['geometry'].x,
                                                                                 vessel.env.FG.nodes[edge[0]]['geometry'].y,
                                                                                 edge_fwd_azimuth, berth_position)
        ...
        # Else if terminal is of type 'jetty': log in logfile of terminal calculating keeping track of number of jetties to be occupied
            ...
            terminal.log_entry("Arrival of vessel", vessel.env.now, len(terminal.resource.users),nx.get_node_attributes(vessel.env.FG, "geometry")[index], )

        # Calculate if the vessel has to wait due to be ready for departure beyond an available tidal window
        vessel.waiting_time = vessel.waiting_time_after_terminal
        ...
        if vessel.berth.type == 'jetty':
            if vessel.berth.resource.queue:
        ...
        if 'original_origin' in dir(vessel):
            vessel.route = nx.dijkstra_path(vessel.env.FG, vessel.route[vessel.route.index(edge[1])], vessel.original_origin)
        else:
