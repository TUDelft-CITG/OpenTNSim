...
def add_energy_attributes_to_eventtable(df, objs):
    ...
    for index, row in df.iterrows():
        ...
        # df.at[index, "engine age (year)"] = obj.C_year # not needed
        ...
        # Test H2 consumption
        # df.at[index, "total_LH2_consumption_PEMFC_mass"] = delta_LH2_PEMFC_mass
...
def add_fuel_attributes_to_event_table(df, objs):
    ...
    # Trying to include the H2 attributes
    # %% ADD H2 ATTRIBUTES INTO EVENT TABLE
def add_H2_attributes_to_event_table(df, objs):
    """
    Method to add fuel-related attributes to a 'minimum event table' plus
    energy attributes.
    ...
    for index, row in df.iterrows():
        ...
        # diesel_C_year = row["total_energy (kWh)"] * obj.final_SFC_diesel_C_year_ICE_mass
        # New H2 line
        H2_consumption = row["total_energy (kWh)"] * obj.final_SFC_LH2_mass_PEMFC
