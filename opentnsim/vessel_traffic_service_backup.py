...
class VesselTrafficService(graph.HasMultiDiGraph):
    ...
    def __init__(self,FG,hydrodynamic_start_time=None,hydrodynamic_information_path=None,vessel_speed_data_path=None,*args,**kwargs):
        ...
        for edge in FG.edges:
            ...
            edges_info.loc[index, 'Edge'] = edge[:2]
    ...
    def provide_waiting_time_for_outbound_tidal_window(self,vessel,route,delay=0,plot=True):
    ...
    def provide_speed_over_edge(self,vessel,edge):
        ...
        if not restricted_vessel_speeds.empty:
            restricted_vessel_speeds_edge = restricted_vessel_speeds[restricted_vessel_speeds.index == edge]
            if not restricted_vessel_speeds_edge.empty:
                v = restricted_vessel_speeds_edge.Speed.iloc[0]
        if math.isnan(v) or not v:
            v = vessel.v
        return v
    ...
    def provide_speed_over_route(self,vessel,route,edges=[]):
        ...
        indexes = vessel_speed_over_route[vessel_speed_over_route.Speed.isna() | (vessel_speed_over_route.Speed == 0)].index
        if len(indexes):
            for index in indexes:
                vessel_speed_over_route.loc[index,'Speed'] = vessel.v
    ...
    def provide_sailing_time(self,vessel, route, edges=[]):
        ...
        sailing_time_over_route.loc[:,['Distance']] = np.array(sailing_distance_over_route['Distance'])
        sailing_time_over_route.loc[:,['Time']] =  np.array(sailing_time_over_route['Distance'])/ np.array(sailing_time_over_route['Speed'])
    ...
    def provide_tidal_window_restriction(self, vessel, route, node, delay, restriction_type='Vertical'):
        ...
        if restriction_type == 'Horizontal':
            restriction_condition_df = horizontal_tidal_restrictions_condition_df.copy()
            ukc, _, _, _, _, _ = self.provide_ukc_clearance(vessel, node, delay)
        elif restriction_type == 'Vertical':
            restriction_condition_df = vertical_tidal_restrictions_condition_df.copy()
        ...
        to_node_mask = (restriction_condition_df.bound_to.isin(route_after_node)|
        ...
        if restriction_condition_df.empty:
            return restriction, tidal_window
    ...
    def provide_required_water_depth(self,vessel,node,delay=0):
        ukc_s, ukc_p, ukc_r, fwa = np.zeros(4)
        required_water_depth = 0.
        if 'Vertical tidal restriction' in vessel.multidigraph.nodes[node].keys():
            restriction, self.vertical_tidal_restriction = self.provide_tidal_window_restriction(vessel, [node], node, delay, restriction_type='Vertical')
            if self.vertical_tidal_restriction:
                ukc_s = restriction.Restriction.ukc_s
                ukc_p = restriction.Restriction.ukc_p * vessel.T
                ukc_r = restriction.Restriction.ukc_r[0] * (vessel.T - restriction.Restriction.ukc_r[1])
                fwa = restriction.Restriction.fwa * vessel.T
        ...
        return required_water_depth, ship_related_factors
    ...
    def provide_ukc_clearance(self,vessel,node,delay=0):
        ...
        required_water_depth, ship_related_factors = self.provide_required_water_depth(vessel,node,delay)
    ...
    def provide_minimum_available_water_depth_along_route(self,vessel, route, time_start, time_end, delay=0):
        ...
        for route_index, node_name in enumerate(nodes_of_interest):
            ...
            required_water_depth,_ = self.provide_required_water_depth(vessel,node_name,delay)
            water_depth = water_level - MBL
            net_ukc_node = water_depth - required_water_depth
            time_correction = time_correction_index
            time_net_ukc_node = times - time_correction
            net_ukc_node_df = pd.DataFrame(net_ukc_node, index=time_net_ukc_node, columns=[node_name])
            net_ukc = pd.concat([net_ukc,net_ukc_node_df],axis=1)
    ...
    def provide_horizontal_tidal_windows(self, vessel, route, time_start, time_end, delay=0, plot=False):
        ...
        def calculate_horizontal_tidal_window(vessel, time_start_index, time_end_index, current_velocity_station, critical_limits, restriction_type, flood=True, ebb=True,decreasing=False):
            ...
            for column_name in ['Tidal period','Period number']:
                currents[column_name] = currents[column_name].ffill()
                currents[column_name] = currents[column_name].bfill()
            ...
            for tide in tides:
                ...
                if restriction_type.window_method == 'Point-based':
                    ...
                    for (period_time_start, period_info), (period_time_stop, _) in zip(tidal_periods.iloc[:-1].iterrows(), tidal_periods.iloc[1:].iterrows()):
                        ...
                        if not period_info['Tidal period'].find(tide) + 1 or tidal_period.empty:
                            continue
        ...
        for route_index, node_name in enumerate(route):
            if 'Horizontal tidal restriction' in vessel.multidigraph.nodes[node_name].keys():
                ...
                restriction, self.horizontal_tidal_restriction = self.provide_tidal_window_restriction(vessel, route, node_name,sailing_time_to_next_node.Time.sum(),restriction_type='Horizontal')
                if not self.horizontal_tidal_restriction:
                    continue
