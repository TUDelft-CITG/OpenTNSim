...
class HasOutput:
    ...
    def update_route_status_report(self, move_stop=False):
        if not move_stop:
            ...
            for node in self.route:
                ...
                if (
                    "Turning basin" in self.multidigraph.nodes[node]
                    and self.bound == "inbound"
                    and self.multidigraph.nodes[node]["Turning Basin"][0].length <= self.L
                ):
    ...
    def update_sailing_status_report(self, current_node, next_node, edge):
        ...
        if (
            "Terminal" in self.multidigraph.edges[edge].keys()
            and self.metadata["terminal_of_call"].size
            and self.metadata["terminal_of_call"][0] in self.multidigraph.edges[edge]["Terminal"].keys()
        ):
        ...
        if "hydrodynamic_data" in dir(self.env.vessel_traffic_service):
            self.output["MBL"], self.output["water_level"], _ = self.env.vessel_traffic_service.provide_water_depth(
                self, current_node
            )

        # Rule-dependent vessel output
        if "Vertical tidal restriction" in self.multidigraph.nodes[current_node].keys():
            self.output["net_ukc"], self.output["gross_ukc"], _, _, self.output["ship_related_ukc_factors"], _ = (
                self.env.vessel_traffic_service.provide_ukc_clearance(self, current_node)
            )
        ...
        if "Horizontal tidal restriction" in self.multidigraph.nodes[current_node].keys():
            time_index = np.absolute(
                ...
                - pd.Timestamp(datetime.datetime.fromtimestamp(self.env.now, tz=pytz.utc)).to_datetime64()
            ).argmin()
            _, self.output["limiting current velocity"] = self.env.vessel_traffic_service.provide_governing_current_velocity(
                self, current_node, time_index, time_index + 1
            )
