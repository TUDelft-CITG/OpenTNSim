                ...
                for lock in locks:
                    ...
                    for r in self.route[loc:]:        
                        if 'Line-up area' in self.env.FG.nodes[r].keys():
                            ...
                            if len(lock2.length.get_queue) != 0:
                                lock2.length.get_queue[-1].id = self.id
                                lock2.length.get_queue[-1].length = self.length
                                lock2.length.get_queue[-1].k = 0
                                total_length_vessels_in_line_up_area = 0
                                k = 0
                                for j in range(len(lock2.length.get_queue)):
                                    if lock2.length.get_queue[j].k == 1:
                                        total_length_vessels_in_line_up_area += lock2.length.get_queue[j].amount-lock2.length.capacity+lock2.length.get_queue[j].length
                                    else:
                                        total_length_vessels_in_line_up_area += lock2.length.get_queue[j].amount
                                    
                                    if lock2.length.get_queue[j].id == self.id and total_length_vessels_in_line_up_area > k*lock2.length.capacity:
                                        k += 1
                                        lock2.length.get_queue.remove(lock2.length.get_queue[j])
                                        access_lineup_length = lock2.length.get(lock2.length.capacity)
                                        lock2.length.get_queue[j].id = self.id
                                        lock2.length.get_queue[-1].length = self.length
                                        lock2.length.get_queue[-1].k = 1
                                        yield access_lineup_length
                                        k -= 1
                                        access_lineup_length = lock2.length.put(lock2.length.capacity-self.length)
                                    elif total_length_vessels_in_line_up_area > k*lock2.length.capacity:
                                        k += 1
                                    
                                if lock2.length.get_queue != [] and lock2.length.get_queue[-1].k == 0:
                                    yield access_lineup_length
            ...
            if "Lock" in self.env.FG.nodes[origin].keys():
                ...
                for lock in locks:
                    if lock.name == self.lock_name:
                        ...
                        if waiting_for_lock_departure != self.env.now:
                            exit_waiting_time = self.env.now-waiting_for_lock_departure
                        else:
                            exit_waiting_time = 0   
