...
def test_islockmaster_init(env, graph):
    ...
    lock_complex = IsLockComplex(node_A="A", node_B="B", name="test lock", env=env)
...
def test_islockcomplex_invalid_nodes(env, graph):
    ...
    with pytest.raises(ValueError, match="LockComplex test lock has invalid node_A X or node_B B which are not part of the graph."):
        lock = IsLockComplex(node_A="X", node_B="B", name="test lock", env=env, start_node="A", end_node="B")

    with pytest.raises(ValueError, match="LockComplex test lock has invalid node_A A or node_B Y which are not part of the graph."):
        lock = IsLockComplex(node_A="A", node_B="Y", name="test lock", env=env, start_node="A", end_node="B")

    with pytest.raises(ValueError, match="does not have an edge"):
        lock = IsLockComplex(node_A="A", node_B="A", name="test lock", env=env, start_node="A", end_node="B")
