...
def test_basic_simulation():
    ...
    # durations
    np.testing.assert_almost_equal(lock_1.doors_open+lock_1.doors_close+2*lock_1.lock_width*lock_1.lock_length*wlev_dif[1][0]/(lock_1.disch_coeff*lock_1.opening_area*math.sqrt(2*lock_1.grav_acc*lock_1.opening_depth))+
                                   distances[0]/vessel_speeds[v][0]+distances[1]/vessel_speeds[v][1]+(distances[2]-0.5*vessels[v].L)/vessel_speeds[v][1]+(0.5*vessels[v].L+distances[3])/vessel_speeds[v][3]+((distances[4]+distances[5])-0.5*vessels[v].L)/vessel_speeds[v][4],
                                   lock_cycle_start, decimal=0, err_msg='', verbose=True)

    np.testing.assert_almost_equal(distances[0]/vessel_speeds[v][0]+distances[1]/vessel_speeds[v][1]+(distances[2]-0.5*vessels[v].L)/vessel_speeds[v][1],
