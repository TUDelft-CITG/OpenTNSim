...
def test_basic_simulation():
    ...
    for v in range(len(vessels)):
        for t in range(0,len(vessels[v].log["Message"])):
            if "node" in vessels[v].log["Message"][t] and "stop" in vessels[v].log["Message"][t]:
                vessel_speed.append(calculate_distance([vessels[v].log['Geometry'][t].x,vessels[v].log['Geometry'][t].y],
    ...
    np.testing.assert_almost_equal(lock_1.doors_open+lock_1.doors_close+2*lock_1.lock_width*lock_1.lock_length*wlev_dif[1][0]/(lock_1.disch_coeff*lock_1.opening_area*math.sqrt(2*lock_1.grav_acc*lock_1.opening_depth)),
                                   lock_cycle_duration, decimal=0, err_msg='', verbose=True)
    ...
    # durations
    np.testing.assert_almost_equal(lock_1.doors_open+lock_1.doors_close+2*lock_1.lock_width*lock_1.lock_length*wlev_dif[1][0]/(lock_1.disch_coeff*lock_1.opening_area*math.sqrt(2*lock_1.grav_acc*lock_1.opening_depth))+
                                   distances[0]/vessel_speeds[v][0]+distances[1]/vessel_speeds[v][1]+(distances[2]-0.5*vessels[v].length)/vessel_speeds[v][1]+(0.5*vessels[v].length+distances[3])/vessel_speeds[v][3]+((distances[4]+distances[5])-0.5*vessels[v].length)/vessel_speeds[v][4],
                                   lock_cycle_start, decimal=0, err_msg='', verbose=True)
    
    np.testing.assert_almost_equal(distances[0]/vessel_speeds[v][0]+distances[1]/vessel_speeds[v][1]+(distances[2]-0.5*vessels[v].length)/vessel_speeds[v][1],
                                   waiting_in_lineup_start, decimal=0, err_msg='', verbose=True)
