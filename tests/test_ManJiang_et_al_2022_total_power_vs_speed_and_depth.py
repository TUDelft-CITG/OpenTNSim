...
def test_simulation():
    ...
    # todo: this list with asserts should be expanded (now it only tests the start of Section 1, also include Section 2?)
    np.testing.assert_almost_equal(250.7395, plot_data['V_s = 3.0 P_tot_given = None P_tot'][0], decimal=0, err_msg='not almost equal', verbose=True)
    np.testing.assert_almost_equal(343.2331, plot_data['V_s = 3.5 P_tot_given = None P_tot'][0], decimal=0, err_msg='not almost equal', verbose=True)
    np.testing.assert_almost_equal(494.1465, plot_data['V_s = 4.0 P_tot_given = None P_tot'][0], decimal=0, err_msg='not almost equal', verbose=True)

    # test if power2v actually results in the right power and the right velocity
