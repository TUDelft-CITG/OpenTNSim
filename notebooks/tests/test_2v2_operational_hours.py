...
operational_hour_start_times = np.arange(pd.Timestamp('2025-01-01 00:00:00'),pd.Timestamp(t_stop),pd.Timedelta(hours=12))
operational_hour_stop_times = np.arange(pd.Timestamp('2025-01-01 01:00:00'),pd.Timestamp(t_stop),pd.Timedelta(hours=12))
...
lock = lock_module.IsLockComplex(env=env,
                                 ...
                                 detector_nodes = ['A','B'],
                                 operational_hour_start_times = operational_hour_start_times,
                                 operational_hour_stop_times = operational_hour_stop_times,
...
def test_results():
    ...
    np.testing.assert_equal(len(lock_df),24)
    assert lock_df.loc[14].Timestamp-lock_df.loc[9].Timestamp > pd.Timedelta(hours=11)
