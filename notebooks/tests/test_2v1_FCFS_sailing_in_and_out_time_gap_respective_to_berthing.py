...
lock = lock_module.IsLockComplex(env=env,
                                 ...
                                 sailing_out_time_gap_through_doors = 300,
                                 sailing_in_time_gap_after_berthing_previous_vessel=300,
                                 sailing_out_time_gap_after_berthing_previous_vessel=300,
...
vessel_3 = Vessel(**{"env": env,
                     ...
                     "C_year": 2010,
                     "L_w":3,
                     "arrival_time": pd.Timestamp('2025-01-01 00:01:00')})
...
def test_results():
    ...
    np.testing.assert_almost_equal(lock.operation_planning.iloc[0].time_entry_stop.round('s').to_pydatetime().timestamp()/10,
                                   vessel_df3.loc[6].Timestamp.to_pydatetime().timestamp()/10,decimal=0)
    ...
    np.testing.assert_almost_equal(lock.operation_planning.iloc[0].time_departure_stop.round('s').to_pydatetime().timestamp()/10,
                                   vessel_df3.loc[10].Timestamp.to_pydatetime().timestamp()/10,decimal=0)
    ...
    assert np.round(vessel_df3.loc[5].Timestamp.to_pydatetime().timestamp()-vessel_df1.loc[3].Timestamp.to_pydatetime().timestamp()) >= lock.sailing_in_time_gap_after_berthing_previous_vessel
